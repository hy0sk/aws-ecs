...
import os
import bcrypt  # 🚀 비밀번호 암호화 라이브러리 추가!
...
class User(Base):
    __tablename__ = "users"
    id = Column(Integer, primary_key=True, index=True)
    username = Column(String(50), unique=True, index=True)  # 아이디 중복 금지
    password = Column(String(255))  # 암호화된 비밀번호 저장 공간
...
def hash_password(password: str) -> str:
    # 비밀번호를 알 수 없는 해시 문자열로 변환 (예: $2b$12$e9... 이런 식으로 바뀜)
    return bcrypt.hashpw(password.encode('utf-8'), bcrypt.gensalt()).decode('utf-8')

# ==========================================
# 🌟 프론트엔드 UI (기존 커뮤니티 게시판 유지) 🌟
# ==========================================
    ...
    <body class="bg-slate-100 font-sans min-h-screen pb-12">
                ...
                <div id="listView" class="block">
                    <table class="w-full text-sm text-left text-slate-600">
                        <thead class="text-xs text-slate-500 bg-slate-100 border-b border-slate-200">
                            <tr><th class="px-4 py-2.5 w-16 text-center font-semibold">번호</th><th class="px-4 py-2.5 font-semibold">제목</th></tr>
                        </thead>
        ...
        <script>
            ...
            function renderPosts() {
                ...
                if (currentPosts.length === 0) { list.innerHTML = '<tr><td colspan="2" class="text-center py-8 text-slate-400">등록된 게시글이 없습니다.</td></tr>'; return; }
            ...
            function router() {
                ...
                if (hash === '#write') {
                    ...
                    document.getElementById('readId').innerText = `글번호: ${post.id}`; document.getElementById('readContent').innerText = post.content;
            ...
            document.getElementById('postForm').addEventListener('submit', async (e) => {
                e.preventDefault(); const title = document.getElementById('title').value; const content = document.getElementById('content').value;
                try {
                    if (currentEditId) { await fetch(`/posts/${currentEditId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title, content }) }); } 
                    else { await fetch('/posts/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title, content }) }); }
                    await loadPosts(); window.location.hash = '';
                } catch (e) { console.error('Error:', e); }
            });
            window.onload = async () => { await loadPosts(); router(); };
        </script>
    </body>
    </html>
    """
...
def signup(user: UserCreate):
    ...
    return {
        "message": "🎉 회원가입 성공! AWS RDS에 암호화되어 안전하게 저장되었습니다.",
        "username": db_user.username,
        "hashed_password_sample": hashed_pw[:15] + "..." # 암호화 결과물 구경용 샘플
    }